...
router.post('/quotations', async (req, res) => {
    try {
        const { customer_id, quotation_number, date, valid_until, status, notes, total, gct, discount } = req.body;
        const pool = getPool();
        const id = randomUUID();
        await pool.request()
            .input('id', sql.UniqueIdentifier, id)
...
router.post('/invoices', async (req, res) => {
    try {
        const { customer_id, quotation_id, invoice_number, date, due_date, status, notes, total, gct, credit } = req.body;
        const pool = getPool();
        const id = randomUUID();
        await pool.request()
            .input('id', sql.UniqueIdentifier, id)
            ...
            .query(`
        INSERT INTO invoices (id, customer_id, quotation_id, invoice_number, date, due_date, status, notes, total, credit, created_at, updated_at)
        VALUES (@id, @customer_id, @quotation_id, @invoice_number, @date, @due_date, @status, @notes, @total, @credit, GETUTCDATE(), GETUTCDATE())
      `);
        res.json({ id });
    }
    catch (error) {
...
router.put('/invoices/:id', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`
        UPDATE invoices
        SET customer_id = @customer_id, quotation_id = @quotation_id, invoice_number = @invoice_number, date = @date, due_date = @due_date, status = @status, notes = @notes, total = @total, credit = @credit, updated_at = GETUTCDATE()
        WHERE id = @id
      `);
        res.json({ success: true });
    }
